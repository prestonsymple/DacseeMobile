...
export default  class ItemPerson extends Component {
  render() {
    const { onPressDetail = () => { }, onPressCheck = () => { }, data, isFriendsGroupCell = false } = this.props
    const { _id, friend_id, friend_info, checked } = data
    const { avatars = [{ url: '' }], email, fullName, phoneCountryCode, phoneNo, userId } = friend_info
    let iconName = isFriendsGroupCell ? 'remove' : 'check'
    let iconColor = isFriendsGroupCell && checked ? '#ff2239' : '#e7e7e7'
    let icon = Icons.Generator.Material(iconName, 18, 'white')
    if (isFriendsGroupCell) {
      iconColor = '#ff2239'
    }else {
      iconColor = checked ? '#7ed321' : '#e7e7e7'
      icon = checked ? Icons.Generator.Material(iconName, 18, 'white') : null
    }
    ...
    return (
      <TouchableOpacity onPress={() => onPressDetail()} activeOpacity={.7} style={{ height: 84, backgroundColor: 'white', justifyContent: 'space-between', alignItems: 'center', flexDirection: 'row' }}>
        ...
        <TouchableOpacity onPress={() => onPressCheck()} hitSlop={{ top: 27, left: 40, bottom: 27, right: 0 }} activeOpacity={.7} style={[styles.circle, { backgroundColor: iconColor }]}>
          {icon}
        </TouchableOpacity>
